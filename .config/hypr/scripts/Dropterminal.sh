...
WIDTH_PERCENT=33  # Width as percentage of screen width
HEIGHT_PERCENT=50 # Height as percentage of screen height
X_PERCENT=29      # X position as percentage from left (25% centers a 50% width window)
...
TERMINAL_CMD="$1"
TERMINAL_BIN=$(echo "$TERMINAL_CMD" | awk '{print $1}' | xargs basename)
...
is_terminal_window() {
    ...
    hyprctl clients -j | jq -e --arg ADDR "$addr" --arg BIN "$term_bin" '
        any(.[]; .address == $ADDR and (.class | ascii_downcase | contains($BIN)))
    ' >/dev/null 2>&1
}
...
spawn_terminal() {
    ...
    if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
        if is_terminal_window "$new_addr" "$TERMINAL_BIN"; then
            # Store the address
            ...
            debug_echo "Spawned window address $new_addr does not match terminal class ($TERMINAL_BIN)"
            return 1
        fi
    fi
